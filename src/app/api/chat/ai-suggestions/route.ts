...
👉 If role is "delivery_boy" → generate 3 short WhatsApp-style reply suggestions that a delivery boy 
...
Last message: ${message}`
        ...
        if (!response.ok) {
            return NextResponse.json({ message: data.error?.message || "Error generating content" }, { status: 500 })
        }
